...
拿到项目后，先看你手上是哪一种，照着做就行。

> **从 GitHub 直接下载的是「源码」**（仓库里不含 exe），请直接看 **②**；只有当你拿到的是别人发来的**打包文件夹 / 压缩包**（里面有 `错题集助手.exe`）时，才看 **①**。

### ① 打包版（文件夹或压缩包里有个 `错题集助手.exe`）

1. 如果是 `.zip`，先**解压**到任意目录
2. **双击 `错题集助手.exe`**
3. 等几秒，浏览器会**自动打开** `http://127.0.0.1:5178` —— 这就是程序界面，可以直接用了
   - 首次启动会自动放入 6 道示例错题，先看看效果
   - 启动后会弹一个**黑色命令行窗口，别关它**；关掉 = 程序退出（数据已自动保存）
   - **不需要装 Node.js**，双击就能跑

### ② 源码版 · Windows（一堆 `.js` 文件，里面有 `server.js`）

1. 先装 [Node.js](https://nodejs.org)（选 LTS 版，18 或更高）
2. **双击 `启动错题集助手.bat`**
3. 浏览器自动打开 → 开始用。**关掉窗口 = 停止**；想后台静默启动就双击 `启动错题集助手（无窗口）.vbs`

### ③ 源码版 · macOS / Linux

1. 先装 [Node.js](https://nodejs.org)（macOS 也可以用 Homebrew：`brew install node`）
2. 在「终端」里进入项目文件夹，运行：
...
https://xxxx-你的公网IP.free.pinggy.net/?code=123456
```

（上面是示例。真实网址每次运行都不同，且 SSH 通道会把你的公网 IP 拼进域名里 ——
**别把真实网址写进公开的地方**，也别提交到版本库。）
...
| 文件 | 平台 | 作用 |
|---|---|---|
| **`启动错题集助手.bat`** | Windows | 启动服务并自动打开浏览器。**关闭窗口 = 停止服务**（数据已自动保存）。 |
| **`启动错题集助手（无窗口）.vbs`** | Windows | 后台静默启动，不弹黑窗口。 |
| **`停止错题集助手.bat`** | Windows | 结束后台运行的服务。 |
| **`start.sh`** | macOS / Linux | 在终端里运行 `bash start.sh`：自动找 node、校验版本、启动服务并打开浏览器。**按 Ctrl+C 或关掉窗口 = 停止服务**。 |
| **`启动错题集助手.command`** | macOS | 双击入口（内部就是调用 `start.sh`），省得自己开终端。 |
...
├── 停止错题集助手.bat            ← 停止后台服务
├── start.sh               macOS / Linux 启动脚本（终端里 bash start.sh）
├── 启动错题集助手.command   macOS 双击启动入口（内部调用 start.sh）
...
- **exe 目前只支持 Windows x64**，因为它是由本机的 `node.exe` 复制而来。要出 macOS / Linux 版本，需要在对应平台（或在 CI 里）用该平台的 node 跑一次 `build-exe.js`。**macOS / Linux 用户不必等打包版**：装好 Node 后 `bash start.sh` 即可使用，功能完全一样，只是少了「免安装」这一条。
